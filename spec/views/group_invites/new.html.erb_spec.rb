...
RSpec.describe "group_invites/new", type: :view do
  before(:each) do
    assign(:group_invite, GroupInvite.new(
      :group => nil,
      :profile => nil,
      :status => "MyString",
      :created_by => 1,
  ...
  it "renders new group_invite form" do
    render

    assert_select "form[action=?][method=?]", group_invites_path, "post" do
